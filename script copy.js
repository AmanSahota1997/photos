...
function displayPhotos(sortKey) {
  // clear previous DOM so counts update cleanly
  gallery.innerHTML = '';

  // 1) Build the pool in the desired order (last => first)
  const pool = photos.slice().reverse(); // last to first

  // 2) limit overall number of photos using the count slider
  const limitedPool = pool.slice(0, imageCountLimit);

  // 3) group the LIMITED pool by the sortKey (so clusters still form)
  const groups = {};
  ...
  // cluster positioning (unchanged, but uses number of groups present)
  ...
  // for each group, place photos
  groupKeys.forEach((key, i) => {
    const cluster = clusterPositions[i % clusterPositions.length];

    // use the group's array (already limited by overall slice)
    ...
    // create/update DOM elements with animation (we cleared gallery so they'll be new)
    groupPhotos.forEach((photo, index) => {
      ...
      if (!el) {
        ...
        // mobile: attach touch click handler so expansion toggles class
        if (isTouchDevice) {
          el.addEventListener('click', () => {
            ...
            if (expanded) {
              ...
              // stop drifting on expand (defensive)
              el.dataset.dx = 0;
              el.dataset.dy = 0;
            } else {
        ...
        // if an element already exists (unlikely after clearing) update it
        el.style.left = pos.x + 'px';
        el.style.top = pos.y + 'px';
        el.style.width = pos.size + 'px';
        el.dataset.cx = pos.x;
